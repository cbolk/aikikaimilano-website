  ...
  class aikidoka {
	...
	function get($dbconn, $aid){
        ...
        if($numrows == 1){
	  		...
	  		$this->hoursfromexam = $this->getTotHoursFromExam($dbconn,$this->id);
	...
	function addExam($dbconn){
  	...
  	function rawlist($dbconn, $activeonly, $beginnersfirst){
  		if($beginnersfirst)
  			$sorting = "ORDER BY beginner, lastname, firstname ASC;";
  		else
  			$sorting = "ORDER BY lastname, firstname ASC;";
  		if($activeonly)			
			$query = "SELECT * FROM aikidoka where active = 1 " . $sorting;
		else
			$query = "SELECT * FROM aikidoka " . $sorting;
	...
	function getExams($dbconn, $aid){
		$query = "SELECT * from exam where aikidoka_fk=" . $aid . " order by date asc;";
		$dbconn->dbconnect();
        $result = $dbconn->qry($query);
  	...
  	function getHoursFromExam($dbconn, $aid){
		...
		for($i = 0; $i < $num; $i++) {
			$mhours[$i] = mysql_fetch_assoc($result);
			if($mhours[$i]['MM']>=9){
				$mhours[$i]['MA'] = $mhours[$i]['MM'] - 9;
				$mhours[$i]['YA'] = $mhours[$i]['YY'] . "-" . ($mhours[$i]['YY'] + 1);
			} else {
				$mhours[$i]['MA'] = $mhours[$i]['MM'] + 3;
				$mhours[$i]['YA'] = ($mhours[$i]['YY'] - 1) . "-" . $mhours[$i]['YY'];
			}
       	}
       	//print_r($mhours);
  		return $mhours;
  	...
  	//active aikidoka only
  	function getHoursFromExamsAllAikidokas($dbconn)
  	{
  		$query = "SELECT k.lastname, k. firstname, k.enrolled, k.last_exam, k.rank, aikidoka_fk, sum(hours) AS tothours, beginner FROM `attendance` a ";
  	...
  	function getHoursYearAllAikidokas($dbconn, $fromyear, $toyear){
		$query = "SELECT k.lastname, k. firstname, k.enrolled, k.last_exam, k.rank, aikidoka_fk, SUM(hours) AS tothours, beginner, ";
		$query = $query . " IF(MONTH(date) >= 9, CONCAT(YEAR(DATE), '-', YEAR(DATE)+1), CONCAT(YEAR(DATE)-1, '-', YEAR(DATE))) as AA FROM attendance a ";
  		$query = $query . " INNER JOIN aikidoka k ON a.aikidoka_fk = k.id ";
  	...
  	function getTotHoursFromExam($dbconn, $aid){
  		$this->lastexam = $this->getDateLastexam($dbconn, $aid);
		$query = "SELECT SUM( hours ) AS MH FROM attendance where aikidoka_fk=" . $aid;
		$query = $query . " AND date > '$this->lastexam' ";
		//echo $query;
		$dbconn->dbconnect();
        $result = $dbconn->qry($query);
  	...
  	function getHoursFromStart($dbconn, $aid){
  		$this->lastexam = $this->getDateLastexam($dbconn, $aid);
		$query = "SELECT YEAR( date ) AS YY, MONTH( date ) AS MM, SUM( hours ) AS MH FROM attendance where aikidoka_fk=" . $aid;
		$query = $query . " GROUP BY YEAR( date ) , MONTH( date )";
		//echo $query;
